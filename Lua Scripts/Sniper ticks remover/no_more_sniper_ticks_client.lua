...
function OnMapLoad()
	...
	local sniper_tick_bitmap_ids = {}
	for k, v in pairs(sniper_tick_bitmaps) do
		local tag = get_tag("bitmap", sniper_tick_bitmaps[k])
		if tag ~= nil then
	...
	if bitmap_count > 0 then
		...
		for k, v in pairs(weapon_hud_interfaces) do
			local static_element_count = read_i32(weapon_hud_interfaces[k] + 96)
			local static_element_ptr = read_u32(weapon_hud_interfaces[k] + 96 + 4)
			
			if static_element_count > 0 then
				for i=0,(static_element_count+1) do
					local cur_stat_element = static_element_ptr + 180 * i
					local cur_stat_element_tag_id = read_u32(cur_stat_element + 72 + 12)
